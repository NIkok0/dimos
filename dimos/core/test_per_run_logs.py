...
from dimos.robot.cli import dimos
...
class TestLogFilePathRouting:
    ...
    def test_startup_logger_after_run_dir_writes_main_jsonl(self, tmp_path):
        log_dir = tmp_path / "run-005"
        set_run_log_dir(log_dir)
        ...
        assert "Starting DimOS" in main_log.read_text()

    def test_cli_configures_run_log_dir_before_startup_log(self):
        run_source = inspect.getsource(dimos.run)

        assert run_source.index("set_run_log_dir(log_dir)") < run_source.index(
            'logger.info("Starting DimOS"'
        )
