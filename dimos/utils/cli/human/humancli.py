...
class HumanCLIApp(App):  # type: ignore[type-arg]
    ...
    def __init__(self, *args, **kwargs) -> None:  # type: ignore[no-untyped-def]
        ...
        self._reasoning_transport = pLCMTransport("/agent_reasoning")  # type: ignore[var-annotated]
        self.chat_log: RichLog | None = None
        self.reasoning_log: RichLog | None = None
        ...
        with Vertical(id="main"):
            with Container(id="chat-container"):
                self.chat_log = RichLog(
                    highlight=True, markup=True, wrap=False, id="chat-log"
                )
                yield self.chat_log

            with Container(id="reasoning-container"):
                self.reasoning_log = RichLog(
                    highlight=True, markup=True, wrap=False, id="reasoning-log"
                )
                yield self.reasoning_log
    ...
    def on_mount(self) -> None:
        ...
        # Set custom highlighter for RichLogs
        self.chat_log.highlighter = JSONHighlighter()  # type: ignore[union-attr]
        self.reasoning_log.highlighter = JSONHighlighter()  # type: ignore[union-attr]

        # Add header to reasoning log
        self.reasoning_log.write(f"[{theme.ACCENT}]Agent Reasoning Stream[/]")
        self.reasoning_log.write("")  # Empty line after header
        ...
        # Reasoning trace display - fixed version
        self._reasoning_thread = threading.Thread(
            target=self._subscribe_to_reasoning, daemon=True
        )
    ...
    def _subscribe_to_reasoning(self) -> None:
        """Subscribe to agent reasoning trace for real-time thinking display."""

        def receive_reasoning(msg: dict[str, Any]) -> None:
            if not self._running:
                return
            ...
            label = step_labels.get(step_type, "•")

            # Format: [timestamp] label [node] content
            timestamp = datetime.now().strftime("%H:%M:%S")
            prefix = f"{label} [{step_type}]"
            if node:
                prefix += f" ({node})"

            # Display with dim styling to differentiate from main conversation
            self.call_from_thread(
                self._add_reasoning_step, timestamp, prefix, content
            )

        self._reasoning_transport.subscribe(receive_reasoning)

    def _add_reasoning_step(self, timestamp: str, prefix: str, content: str) -> None:
        """Add a reasoning step to the reasoning log with dim/italic styling.

        Reasoning messages are displayed in a dedicated area separate from
        the main chat log, allowing real-time streaming of agent thinking.
        """
        # Format timestamp
        time_parts = timestamp.split(":")
        if len(time_parts) == 3:
            timestamp_formatted = f"[{theme.DIM}]{time_parts[0]}:{time_parts[1]}:{time_parts[2]}[/{theme.DIM}]"
        else:
            timestamp_formatted = f"[{theme.DIM}]{timestamp}[/{theme.DIM}]"

        # Format prefix with accent color
        prefix_formatted = f"[{theme.ACCENT}]{prefix}[/{theme.ACCENT}]"

        # Content with dim styling and italics
        # Truncate very long content
        display_content = content[:150] + "..." if len(content) > 150 else content
        content_formatted = f"[dim italic]{display_content}[/dim italic]"

        full_line = f"{timestamp_formatted} {prefix_formatted} {content_formatted}"

        assert self.reasoning_log is not None
        self.reasoning_log.write(full_line)
        # Auto-scroll to show latest reasoning
        self.reasoning_log.scroll_end(animate=False)
