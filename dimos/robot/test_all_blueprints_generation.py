...
def test_production_registry_excludes_demo_entries() -> None:
    """Keep demo blueprints and modules out of the production run registry."""
    from dimos.robot.all_blueprints import all_blueprints, all_modules

    demo_blueprints = [name for name in all_blueprints if name.startswith("demo-")]
    demo_modules = [
        name
        for name, path in all_modules.items()
        if name.startswith("demo-") or ".demo_" in path or ".demos." in path
    ]

    assert demo_blueprints == []
    assert demo_modules == []
...
def _scan_for_blueprints(root: Path) -> tuple[dict[str, str], dict[str, str]]:
    ...
    for file_path in sorted(_get_all_python_files(root)):
        if _is_demo_file(file_path, root):
            continue
        ...
        for var_name in blueprint_vars:
            if _is_demo_registry_name(var_name):
                continue
            full_path = f"{module_name}:{var_name}"
            cli_name = _blueprint_cli_name(var_name)
        ...
        if _is_production_module_file(file_path, root):
            for class_name in module_vars:
                cli_name = class_name_to_registry_key(class_name)
                if _is_demo_registry_name(cli_name) or _is_demo_registry_name(class_name):
                    continue
...
def _is_demo_file(file_path: Path, root: Path) -> bool:
    """Return whether a source file is a demo-only entrypoint."""
    rel = str(file_path.relative_to(root))
    stem = file_path.stem
    return (
        stem.startswith("demo")
        or "/demos/" in rel
        or "/demo" in rel
        or rel.startswith("core/tests/")
        and stem.startswith("demo")
    )


def _is_demo_registry_name(name: str) -> bool:
    """Return whether a blueprint or module registry name is demo-only."""
    normalized = name.replace("_", "-").lower()
    return normalized.startswith("demo-") or normalized.startswith("demo")


def _blueprint_cli_name(var_name: str) -> str:
    """Return the public CLI name for a blueprint variable."""
