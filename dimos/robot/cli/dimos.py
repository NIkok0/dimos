...
def run(
    ...
    logger.info("Starting DimOS", run_id=run_id, blueprint=blueprint_name, log_dir=str(log_dir))
...
def log_cmd(
    ...
    json_array: bool = typer.Option(False, "--json-array", help="Output as JSON array (for easy reading)"),
    run_id: str = typer.Option("", "--run", "-r", help="Specific run ID"),
) -> None:
    """View logs from a DimOS run."""
    from dimos.core.log_viewer import follow_log, format_line, read_log, resolve_log_path, format_as_json_array
    ...
    # JSON array mode: read all lines and output as a single JSON array
    if json_array:
        count = None if all_lines else lines
        lines_list = read_log(path, count)
        typer.echo(format_as_json_array(lines_list))
        return
...
from dimos.agents.cli.nav_app import app as nav_app

main.add_typer(nav_app, name="nav")

from dimos.agents.cli.nav_mcp_app import app as nav_mcp_app
