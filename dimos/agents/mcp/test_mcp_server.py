...
from dimos.agents.mcp.mcp_server import _agent_message_sse_data, app, handle_request
...
def test_agent_message_sse_data_includes_reasoning_content() -> None:
    msg = AIMessage(
        content="你好",
        additional_kwargs={"reasoning_content": "用户在打招呼"},
    )
    data = _agent_message_sse_data(msg)
    assert data["type"] == "ai"
    assert data["content"] == "你好"
    assert data["reasoning_content"] == "用户在打招呼"


def test_agent_message_sse_data_omits_reasoning_when_absent() -> None:
