...
class McpClient(Module):
    ...
    human_input: In[str]
    agent_idle: Out[bool]
    agent_reasoning: Out[dict]  # Stream of agent reasoning steps for humancli
    ...
    def on_system_modules(self, _modules: list[RPCClient]) -> None:
        ...
        with self._lock:
            chat_model = make_chat_model(model) if isinstance(model, str) else model
    ...
    def _log_reasoning_step(self, step_type: str, content: str, node: str = "", **extra: Any) -> None:
        """Log and stream an agent reasoning step.

        Publishes to both structured logs (for dimos log) and reasoning_trace
        stream (for humancli real-time display).

        Args:
            step_type: Type of reasoning step - "thought", "action", "observation", "plan"
            content: The reasoning content/text
            node: Which LangGraph node produced this step (e.g., "agent", "tools")
            **extra: Additional fields to include in the log
        """
        # Always stream to humancli (real-time display)
        # Logs are controlled by DIMOS_AGENT_DEBUG_STREAM
        ...
        if extra:
            reasoning_data.update(extra)

        # Publish to agent_reasoning stream for humancli
        self.agent_reasoning.publish(reasoning_data)

        # Also log if debug mode enabled
        if os.getenv("DIMOS_AGENT_DEBUG_STREAM"):
            log_data = {
                "trace_layer": "agent_reasoning",
                "trace_stage": step_type,
                "step_type": step_type,
                "node": node,
                "content_preview": preview,
            }
            ...
            logger.info("Agent reasoning step", **log_data)
    ...
    def _process_message(
        ...
        # Determine stream mode based on debug setting
        debug_mode = os.getenv("DIMOS_AGENT_DEBUG_STREAM")
        stream_mode = "debug" if debug_mode == "full" else "updates"

        self._log_reasoning_step(
            "plan",
            f"Starting agent execution with stream_mode={stream_mode}",
            node="",
            message_type=message.type if hasattr(message, "type") else "unknown",
        )

        for chunk in state_graph.stream({"messages": self._history}, stream_mode=stream_mode):
            # In debug mode, log raw chunk for deep inspection
            if debug_mode == "full":
                logger.debug(
                    "Agent stream chunk",
                    trace_layer="agent_reasoning",
            ...
            for node_name, node_output in chunk.items():
                ...
                # 1. Model node (langchain create_agent): thinking + tool selection
                ...
                # 2. Tools node: contains observations/results
                ...
                # 3. Any node: standard message handling
                for msg in node_output.get("messages", []):
                    self._history.append(msg)
                    pretty_print_langchain_message(msg)
                    self.agent.publish(msg)

        self._log_reasoning_step("plan", "Agent execution completed", node="")
