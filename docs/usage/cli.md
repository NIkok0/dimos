...
| `--robot-width` | FLOAT | `0.54` | Robot width in meters (seerKiva default) |
| `--robot-length` | FLOAT | `0.778` | Robot length in meters (seerKiva default) |
| `--robot-rotation-diameter` | FLOAT | `1.0` | Robot rotation diameter in meters |
| `--ros-nav-target-safety-mode` | `circle\|footprint` | `footprint` | Pre-goal map safety geometry |
| `--ros-nav-target-safety-radius-m` | FLOAT | `0.585` | Circle-mode safety radius (rotation radius + collision offset) |
| `--ros-nav-collision-offset-m` | FLOAT | `0.085` | Footprint inflation matching Seer「碰撞偏移量」 |
...
### Navigation target safety (dax-agent / ros_topic nav)

DimOS checks `/map` **before** sending `/navigate_to_pose`. Default geometry matches **seerKiva** (540×778 mm chassis, 85 mm collision offset). Align robot-side Nav2 / Seer AMR model with the same values:

| Seer AMR 后台 | DimOS flag / env |
|---------------|------------------|
| 长 778 mm | `--robot-length 0.778` / `DIMOS_ROBOT_LENGTH` |
| 宽 540 mm | `--robot-width 0.54` / `DIMOS_ROBOT_WIDTH` |
| 旋转直径 1000 mm | `--robot-rotation-diameter 1.0` |
| 碰撞偏移量 85 mm | `--ros-nav-collision-offset-m 0.085` |

Use `--ros-nav-target-safety-mode circle` with `--ros-nav-target-safety-radius-m 0.585` to revert to circular pre-check.

While waiting for `/navigate_to_pose`, DimOS watches `/navigation_current_status` and auto-cancels on **1005/1006/1007** by default (`--ros-nav-auto-cancel-status-codes`). Disable with `--no-ros-nav-auto-cancel-enabled`.
